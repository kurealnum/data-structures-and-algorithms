...
class BinarySearchTree(BinaryTree):
    ...
    def fill_balanced_tree(self, arr: list) -> None:
        self.root = self.__fill_balanced_tree_helper(arr)
    ...
    def __fill_balanced_tree_helper(self, arr: list) -> Node:
        if not arr:
            return None
        ...
        root.left = self.__fill_balanced_tree_helper(arr[:mid])
        root.right = self.__fill_balanced_tree_helper(arr[mid+1:])
    ...
    def search(self, target_node) -> Node:
        return self.__search_helper(target_node, self.root)

    #make sure to add .key to whatever return value you get, as this just returns the node objec t
    def __search_helper(self, target_node, node: Node) -> bool:
        current = node.key
        if target_node == current:
            return True
        
        if target_node > current:
            return self.__search_helper(target_node, node.right)

        else:
            return self.__search_helper(target_node, node.left)
        


    def insert(self, key) -> Node:
        self.__insert_helper(self.root, key)

    #takes the root and a new node() object as input
    def __insert_helper(self, root: Node, key) -> Node:
        if root is None:
            return Node(key)
            ...
            if root.key == key:
                ...
                root.right = self.__insert_helper(root.right, key)
            else:
                root.left = self.__insert_helper(root.left, key)

        return root


    def remove(self, target_node) -> Node:
        self.__remove_node_helper(self.root, target_node)

    #TBD stands for To Be Removed, takes tree root and new node *value* as input
    def __remove_node_helper(self, root: Node, node_TBD) -> Node:
        ...
        if node_TBD > root.key:
            ...
            root.right = self.__remove_node_helper(root.right, node_TBD)
            return root

        if node_TBD < root.key:
            #assign it incase it changes
            root.left = self.__remove_node_helper(root.left, node_TBD)
            return root
        ...
        #deletion process
            ...
            root.right = self.__remove_node_helper(root.right, root.key)
            return root
